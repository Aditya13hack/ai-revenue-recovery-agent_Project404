...
## Hinglish Voice Recovery (Telephony-Ready)

The agent communicates in natural Hinglish — the way most Indians actually talk about payments. For cases routed to the voice channel, the pipeline generates dynamic, personalized voice outreach:

1. **Contextual Script:** The LLM drafts an empathetic, tailored dialogue using the customer's name, pending amount, and root failure cause.
2. **Neural Audio Synthesis:** The dialogue is converted into high-fidelity Hinglish speech using `edge-tts` with the `hi-IN-MadhurNeural` model.
3. **Dashboard Playback:** Each case features an embedded audio player for reviewing the synthesized call audio.
4. **Live-Call Ready:** The audio generation and streaming endpoints (`GET /api/cases/{id}/audio`) are built to be telephony-ready — connecting live outbound calling via Twilio, Exotel, or Sarvam is simply an API key and webhook configuration away.
